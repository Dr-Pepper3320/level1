...
(function(){
  ...
  function stateKeyFor(data){
    ...
    return "dlg_scripted_state_v2__" + id;
  }
    ...
    try{
      switch(String(action||"")){
        case "start_troll_duel": {
          ...
          // Save a return pack that Level 1 can apply (position restore on win OR lose)
          try{
            const st = window.__LEVEL1_STATE__ || null;
            const p = st && st.player ? st.player : null;
            const pack = {
              type: "duel_return_state",
              encounterId: String(encounterId),
              returnUrl: String(returnUrl),
              ts: Date.now(),
              player: (p && isFinite(p.x) && isFinite(p.y)) ? {
                x: Number(p.x),
                y: Number(p.y),
                faceX: Number(p.faceX || 0),
                faceY: Number(p.faceY || 1)
              } : null
            };
            localStorage.setItem("np_duel_return_state_v1", JSON.stringify(pack));
